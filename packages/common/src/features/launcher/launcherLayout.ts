...
export function derivePageCount(
  ...
  const maxPage = items.reduce(
    (m, it) => Math.max(m, pageOf(it.address, perPage)),
    -1,
  );
...
export function buildCommitted(
  views: StoreItem[],
  persisted: PersistedLayoutV1 | null,
): LauncherItem[] {
  const addressById = new Map<string, number>(
    persisted?.items.map((i) => [i.id, i.address]) ?? [],
  );
...
export function reflowIfNeeded(
  persisted: PersistedLayoutV1 | null,
  currentPerPage: number,
): PersistedLayoutV1 | null {
  if (
    !persisted ||
    currentPerPage <= 0 ||
    persisted.perPage === currentPerPage
  ) {
    return persisted;
  }
