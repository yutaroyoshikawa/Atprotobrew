...
async function editProfile(
  ...
  const avatarBlobRef = input.pendingAvatar
    ? await uploadBlob(agent, input.pendingAvatar)
    : undefined;
  const bannerBlobRef = input.pendingBanner
    ? await uploadBlob(agent, input.pendingBanner)
    : undefined;
