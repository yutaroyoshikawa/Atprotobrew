...
const widgetConfig: WithAndroidWidgetsParams = {
  widgets: [
    {
      name: "LauncherWidgetViewSmall",
      label: "Atprotobrew Launcher (S)",
      minWidth: "110dp",
      minHeight: "110dp",
      targetCellWidth: 2,
      targetCellHeight: 2,
    ...
    {
      name: "LauncherWidgetViewMedium",
      label: "Atprotobrew Launcher (M)",
      minWidth: "110dp",
      minHeight: "110dp",
      targetCellWidth: 4,
      targetCellHeight: 2,
    ...
    {
      name: "LauncherWidgetViewLarge",
      label: "Atprotobrew Launcher (L)",
      minWidth: "110dp",
      minHeight: "110dp",
      targetCellWidth: 3,
      targetCellHeight: 3,
...
const withWidgetMinWidth = (c: ExpoConfig) =>
  withDangerousMod(c as Parameters<typeof withDangerousMod>[0], [
    ...
    async (mod) => {
      ...
      await fs.writeFile(
        xmlPath,
        xml.replace(/android:minWidth="[^"]*"/, 'android:minWidth="110dp"'),
      );
      return mod;
    },
  ]);
...
const withWorkRuntimeFix = (c: ExpoConfig) =>
  withProjectBuildGradle(
    c as Parameters<typeof withProjectBuildGradle>[0],
    (mod) => {
  ...
  withAndroidManifest(c as Parameters<typeof withAndroidManifest>[0], (mod) => {
    ...
    if (app?.receiver) {
      app.receiver = app.receiver.filter(
        (r) => r.$?.["android:name"] !== ".LauncherWidgetProvider",
      );
    }
    return mod;
  });
