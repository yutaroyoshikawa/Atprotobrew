import { isAtprotoDid, type AtprotoDid } from "@atproto/did";
...
import { LogOut, PlusCircle, QrCode, Trash2, User } from "lucide-react";
import { useCallback, useEffect, useId, useRef, useState } from "react";
import { Link } from "react-router-dom";
import {
  ProfileQrCodeDialog,
  type ProfileQrCodeDialogMethods,
} from "./ProfileQrCodeDialog";
  ...
  return (
    <div className="group flex items-center w-full">
      ...
      >
        ...
        <span
          className={`text-sm flex-1 truncate ${isActive ? "font-semibold" : ""}`}
        >
          {displayHandle}
        </span>
...
export function UserMenuButton({
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        ...
        >
          ...
          >
            <PlusCircle
              size={18}
              className="text-textContrastMedium shrink-0"
            />
          ...
          <button
            ...
            className="flex items-center gap-3 w-full px-4 py-3 text-left text-text hover:bg-bgContrast50 transition-colors cursor-pointer border-b border-bgContrast50"
          >
            <QrCode size={18} className="text-textContrastMedium shrink-0" />
            <span className="text-base">QR コードで共有</span>
          </button>
      ...
      {profile && isAtprotoDid(profile.did) && (
        <ProfileQrCodeDialog
          ref={qrDialogRef}
          did={profile.did}
